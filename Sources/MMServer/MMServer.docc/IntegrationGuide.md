...
On(Journal.read, Accepts("tenants.*.journal.*")) { ... }             // any tenant's journal subtree
On(Tree.walk, Accepts(.root, .all)) { ... }                          // root AND any entity
```

The patterns compose (any match admits), and one grammar rule generates them all: a pattern is dot-separated segments, each a literal or `*` (matching exactly one segment — `"tenants.*.journal"` admits `tenants.acme.journal`, not `tenants.a.b.journal`), and a **trailing** `.*` means one or more further segments at any depth. So `"journal.*"` is strict descendants (glob discipline — list `"journal"` too when the namespace entity is itself a target), a bare name is an exact entity, and `"*"` / `.all` is the degenerate case: no prefix, any depth — any non-root entity (the default). `.root` opts into root-targeted dispatches — the only way to accept them, since root carries no ACL; it is reserved for methods with documented tree-wide semantics whose handlers enforce their own authorization (as the builtin `server.schema` does).
...
One ergonomic falls out of the exact form: a route whose vocabulary names **exactly one** concrete entity (`Accepts("system.log")` — one pattern, no wildcards) accepts an **entity-less** request as targeting it. The client omits `on:` (it defaults to root) or the CLI omits the `<entity>` positional; the server infers the target and runs traversal plus the target ACL check on the inferred entity exactly as if it had been spelled. Wider vocabularies infer nothing — an entity-less request there is denied like any other unaccepted target — so omission is never ambiguous, and the inference lives entirely server-side (clients never see `Accepts`; omitting against the wrong daemon is just a denial).
