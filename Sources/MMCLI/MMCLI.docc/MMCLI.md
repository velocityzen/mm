...
- ``MMCLIOptions`` is the shared `@OptionGroup` every command embeds: `--socket`/`--tcp` (exactly one), connect/hello timeouts, `--output` (``OutputFormat``: `json`, `json-pretty`, `raw`), and `--no-verify`.
...
- ``MMCLIFailure`` maps `MMCallError` to sysexits-flavored codes (denied → 77, unknown method → 64, malformed params → 65, transport → 69, SIGINT → 130, application `MMError`s → 1 with `error <code>: <message>` on stderr) and validates entity arguments.
