...
- Calendar/clock fields are first-class wire kinds, never strings: `Field("day", .date)` (calendar date), `.datetime` (wall clock, no zone), `.timestamp` (absolute instant with offset) — ISO 8601 strings on the wire (strict canonical grammar, spec §8.3), `MMDate`/`MMDateTime`/`MMTimestamp` in Swift (dependency-free, `Comparable`; timestamps order by instant). Don't model a date as `.string` or an epoch `.int`.
